...
export class ViewinghistoryService {
  ...
  async handleGetListViewingHistory(query: string, current: number, pageSize: number) {
    ...
    // Tính tổng số lượng
    const totalItems = await this.viewingHistoryModel.countDocuments(filter.query);
    // Tính tổng số trang
